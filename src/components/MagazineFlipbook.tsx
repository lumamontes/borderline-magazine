...
export default function MagazineFlipbook({ pageImages, title, isPreview = false, magazineSlug, colorPalette, coverImage }: MagazineFlipbookProps) {
  ...
  return (
    ...
    >
      {isPreview && pageImages.length > 3 && (
        <div className="text-center">
          <div 
            className="mb-4 p-4 rounded-lg border"
            style={{
              backgroundColor: colorPalette?.background || '#f0f9ff',
              borderColor: colorPalette?.border || '#bfdbfe',
              color: colorPalette?.text || '#1e40af'
            }}
          >
            <p className="text-sm mb-2">
              This is a preview showing the first 3 pages
            </p>
            <a
            href={`/magazine/preview/${magazineSlug}`}
            className="inline-flex items-center px-6 py-3  text-black font-semibold rounded-lg transition-colors duration-200 border border-black shadow-none hover:bg-black hover:text-white "
          >
            <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
            </svg>
            Explore Full Magazine
          </a>
          </div>
